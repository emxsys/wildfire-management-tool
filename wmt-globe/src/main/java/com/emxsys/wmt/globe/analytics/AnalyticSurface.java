 ...
 * {@link Sector} at a specified base altitude in meters. The number of grid locations is defined by
 * the AnalyticSurface's dimensions. The default dimensions are
 * <code>(10, 10)</code>. Callers specify the dimensions by using one of the constructors accepting
 * <code>width</code> and
 * <code>height</code>, or by invoking {@link #setDimensions(int, int)}. Each grid point has the
 * following set of attributes: <ul> <li>Scalar value : the grid point's height relative to the
 * surface's base altitude, both in meters</li> <li>Color : the grid point's RGBA color
 * components</li> </ul> Callers specify the attributes at each grid point by invoking
 * {@link #setValues(Iterable)} with an {@link Iterable} of {@link
 * GridPointAttributes}. Grid points are assigned attributes from this iterable staring at the upper
 * left hand corner, and proceeding in row-first order across the grid. The iterable should contain
 * at least
 * <code>width * height</code> values, where width and height are the AnalyticSurface's grid
 * dimensions. If the caller does not specify any GridPointAttributes, or the caller specified
 * iterable contains too few values, the unassigned grid points are given default attributes: the
 * default scalar value is 0, and the default color is {@link java.awt.Color#BLACK}.
 * <p/>
 * <h2>Surface Altitude</h2>
 * <p/>
 * AnalyticSurface's altitude can vary at each grid point. The altitude of each grid point depends
 * on four properties: the altitude mode, the surface altitude, the vertical scale, and the scalar
 * value from GridPointAttributes. The following table outlines how the altitude at each grid point
 * is computed for each altitude mode:
 * <p/>
 * <table border="1"> <tr><th>Altitude Mode</th><th>Grid Point Altitude</th></tr>
 * <tr><td>WorldWind.ABSOLUTE (default)</td><td>surface altitude + (vertical scale * scalar value
 * from GridPointAttributes)</td></tr>
 * <tr><td>WorldWind.RELATIVE_TO_GROUND</td><td>terrain height at grid point + surface altitude +
 * (vertical scale * scalar value from GridPointAttributes)</td></tr>
 * <tr><td>WorldWind.CLAMP_TO_GROUND</td><td>terrain height at grid point</td></tr> </table>
 * <p/>
 * Note that when the altitude mode is WorldWind.CLAMP_TO_GROUND the surface altitude, vertical
 * scale, and the scalar value from GridPointAttributes are ignored. In this altitude mode only the
 * Sector, dimensions, and color from GridPointAttributes are used.
 *
 * @author dcollins
...
{
    /**
     * GridPointAttributes defines the properties associated with a single grid point of an
     * AnalyticSurface.
     */
    public interface GridPointAttributes
    {
        /**
         * Returns the scalar value associated with a grid point. By default, AnalyticSurface
         * interprets this value as the grid point's height relative to the AnalyticSurface's base
         * altitude, both in meters.
         *
        ...
        /**
         * Returns the {@link java.awt.Color} associated with a grid point. By default,
         * AnalyticSurface interprets this Color as the RGBA components of a grid point's RGBA
         * color.
         *
    ...
    /**
     * Constructs a new AnalyticSurface with the specified {@link Sector}, base altitude in meters,
     * grid dimensions, and iterable of GridPointAttributes. The iterable should contain at least
     * <code>with * height</code> non-null GridPointAttributes.
     *
     ...
     *
     * @throws IllegalArgumentException if the sector is null, if the width is less than 1, if the
     * height is less than 1, or if the iterable is null.
     */
    ...
    /**
     * Constructs a new AnalyticSurface with the specified {@link Sector}, base altitude in meters,
     * and grid dimensions. The new AnalyticSurface has the default {@link GridPointAttributes}.
     *
     * @param sector the Sector which defines the surface's geographic region.
     * @param altitude the base altitude to place the surface at, in meters.
     * @param width the surface grid width, in number of grid points.
     * @param height the surface grid height, in number of grid points.
     *
     * @throws IllegalArgumentException if the sector is null, if the width is less than 1, or if
    ...
    /**
     * Constructs a new AnalyticSurface with the specified {@link Sector} and base altitude in
     * meters. The new AnalyticSurface has default dimensions of
     * <code>(10, 10)</code>, and default {@link GridPointAttributes}.
     *
     * @param sector the Sector which defines the surface's geographic region.
     * @param altitude the base altitude to place the surface at, in meters.
     *
    ...
    /**
     * Constructs a new AnalyticSurface with the specified grid dimensions. The new AnalyticSurface
     * is has the default Sector {@link Sector#EMPTY_SECTOR}, the default altitude of 0 meters, and
    ...
    /**
     * Constructs a new AnalyticSurface with the default Sector {@link Sector#EMPTY_SECTOR}, the
     * default altitude of 0 meters, default dimensions of
     * <code>(10, 10)</code>, and default {@link GridPointAttributes}.
     */
    ...
    /**
     * Sets this surface's base altitude, in meters. See the AnalyticSurface class documentation for
     * information on how the altitude is interpreted.
     *
     ...
     * gov.nasa.worldwind.WorldWind#ABSOLUTE} is used if specified. See the AnalyticSurface class
     * documentation for information on how the altitude mode is interpreted.
     *
    ...
    /**
     * Returns the number of horizontal and vertical points composing this surface as an array with
     * two values. The value at index 0 indicates the grid width, and the value at index 1 indicates
     * the grid height.
     *
     ...
     * {@link #setValues(Iterable)} for details on how this iterable is interpreted by
     * AnalyticSurface.
     *
    ...
    /**
     * Sets this surface's iterable of {@link GridPointAttributes}. Grid points are assigned
     * attributes from this iterable staring at the upper left hand corner, and proceeding in
     * row-first order across the grid. The iterable should contain at least
     * <code>width * height</code> values, where width and height are the AnalyticSurface's grid
     * dimensions. If the iterable contains too few values, the unassigned grid points are given
     * default attributes: the default scalar value is 0, and the default color is
    ...
    /**
     * Sets the scale applied to the value at each grid point. Before rendering, this value is
     * applied to each grid points scalar value, thus increasing or decreasing it's height relative
     * to the surface's base altitude, both in meters.
     *
    ...
    /**
     * Returns a copy of the rendering attributes associated with this surface. Modifying the
     * contents of the returned reference has no effect on this surface. In order to make an
     * attribute change take effect, invoke {@link
    ...
    /**
     * Sets the rendering attributes associated with this surface. The caller cannot assume that
     * modifying the attribute reference after calling setSurfaceAttributes() will have any effect,
     * as the implementation may defensively copy the attribute reference. In order to make an
     * attribute change take effect, invoke setSurfaceAttributes(AnalyticSurfaceAttributes) again
    ...
    /**
     * Returns the object which is associated with this surface during picking. A null value is
     * permitted and indicates that the surface itself will be the object returned during picking.
     *
    ...
    /**
     * Sets the object associated with this surface during picking. A null value is permitted and
     * indicates that the surface itself will be the object returned during picking.
     *
     * @param pickObject the object to associated with this surface during picking. A null value is
     * permitted and indicates that the surface itself will be the object returned during picking.
     */
    ...
    /**
     * Sets the layer associated with this surface during picking. A null value is permitted, and
     * indicates that no layer is associated with this surface.
     *
    ...
    /**
     * Test if this AnalyticSurface intersects the specified draw context's frustum. During picking
     * mode, this tests intersection against all of the draw context's pick frustums. During
     * rendering mode, this tests intersection against the draw context's viewing frustum.
     *
     ...
     * <code>missingDataSignal</code> are ignored. This returns null if the iterable is empty or
     ...
     *
     * @return an array containing the minimum value in index 0 and the maximum value in index 1, or
     * null if the iterable is empty or contains only missing values.
     *
     * @throws IllegalArgumentException if the iterable is null.
     */
    ...
    /**
     * Returns the minimum and maximum values in the specified iterable of
     * {@link GridPointAttributes}. Values equivalent to
     * <code>Double.NaN</code> are ignored. This returns null if the buffer is empty or contains
     * only NaN values.
     *
     * @param iterable the GridPointAttributes to search for the minimum and maximum value.
     *
     * @return an array containing the minimum value in index 0 and the maximum value in index 1, or
     * null if the iterable is empty or contains only NaN values.
     *
     * @throws IllegalArgumentException if the iterable is null.
     */
    public static double[] computeExtremeValues(Iterable<? extends GridPointAttributes> iterable)
     ...
     */
    public static GridPointAttributes createGridPointAttributes(final double value,
    ...
    /**
     * Returns a new instance of {@link GridPointAttributes} with a Color computed from the
     * specified value and value range. The color's RGB components are computed by mapping value's
     * relative position in the range
     * <code>[minValue,
     * maxValue]</code> to the range of color hues
     * <code>[minHue, maxHue]</code>. The color's Alpha component is computed by mapping the
     * values's relative position in the range
     * <code>[minValue, minValue + (maxValue - minValue)
     * / 10]</code> to the range
     * <code>[0, 1]</code>. This has the effect of interpolating hue and alpha based on the grid
     * point value.
     *
     * @param value the new GridPointAttributes' value.
     ...
     *
     * @return a new GridPointAttributes defined by the specified value, value range, and color hue
     * range.
     */
    public static AnalyticSurface.GridPointAttributes createColorGradientAttributes(
        final double value,
    ...
    /**
     * Returns a new iterable populated with the default {@link GridPointAttributes}. The default
     * GridPointAttributes have a value of 0, and the color {@link java.awt.Color#BLACK}.
     *
     ...
     * #createColorGradientAttributes(double, double, double, double, double)} for each double value
     * in the speicfied {@link BufferWrapper}. Values equivalent to the specified
     * <code>missingDataSignal</code> are replaced with the specified
     * <code>minValue</code>.
     *
    ...
    public static Iterable<? extends AnalyticSurface.GridPointAttributes> createColorGradientValues(
        BufferWrapper values, double missingDataSignal, double minValue, double maxValue,
    ...
    protected void updateNextSurfacePoint(DrawContext dc, Angle lat, Angle lon,
        GridPointAttributes attr,
